...
func TestUpdateCustomer(t *testing.T) {
	...
	tests := []struct {
		name        string
		parameter   serializer.UpdatedCustomerReq
		...
		},
		{
			...
			mockService: func(m *mocks.CustomerService) {
				m.On("Update", mock.IsType(model.UpdateCustomerOpts{})).Return(errors.New("error from service"))
			},
			isError: true,
		},
		{
			...
			mockService: func(m *mocks.CustomerService) {
				m.On("Update", mock.IsType(model.UpdateCustomerOpts{})).Return(nil)
			},
			isError: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r.NoError(err)
			req := httptest.NewRequest(http.MethodPatch, "/customer", strings.NewReader(string(reqBody)))
			req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
			rec := httptest.NewRecorder()
			e := echo.New()
			c := e.NewContext(req, rec)
			c.Set("ActionBy", tt.parameter.UpdatedBy.String())
			...
			err = UpdateCustomer(UpdateCustomerCfg{
			...
			if tt.isError {
				...
				r.NoError(err)
				r.Equal(http.StatusAccepted, rec.Result().StatusCode)
			}
		})
	}
}
